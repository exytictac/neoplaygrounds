...
local yearday, hour
local timer = nil
addEvent ( "acclog:updated", true )

function logData ( enabled )
	if isEnabled then
		enabled = 1
		triggerEvent ( "acclog:updated", localPlayer )
		outputChatBox ( "check" )
	else
		enabled = 0
	end
end
addCommandHandler ( "log", logData )

function applyClientConfigSettings( _, enabled)
	if (enabled == 1) then
		openFile( )
		isEnabled = true
	else
...
addEventHandler("acclog:updated", getRootElement(), applyClientConfigSettings)
