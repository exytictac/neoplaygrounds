...
function Animation.presets.guiResize(endWidth, endHeight, time, loop, startWidth, startHeight, speedUpSlowDown)
	-- guiResize(endWidth, endHeight, [ time = 1000, loop = false,
	--   startWidth = current width, startHeight = current height, speedUpSlowDown = false ])
	if type(endWidth) ~= 'userdata' then
		return { from = speedUpSlowDown and -math.pi/2 or 0, to = speedUpSlowDown and math.pi/2 or 1,
		         time = time or 1000, repeats = loop and 0 or 1, transform = math.sin, fn = Animation.presets.guiResize,
		         startWidth = startWidth, startHeight = startHeight,
		         endWidth = endWidth, endHeight = endHeight, speedUpSlowDown = speedUpSlowDown }
	else
		if speedUpSlowDown then
			value = (value + 1)/2
		end
