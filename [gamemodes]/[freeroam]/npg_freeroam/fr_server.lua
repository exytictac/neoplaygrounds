...
addEventHandler ( "onPlayerSpawn", getRootElement(), function()
		...
		if getElementData(source, "freemode") ~= "hay" then
			setTimer(setElementData, 500, 1, source, "freemode", "freeroam", true)
		end
	end
)
...
addEventHandler('onPlayerWasted', root,
	function()
		...
		setTimer(spawnPlayer,4500,1,source,x,y,z,0,getElementModel(source),getElementInterior(source),getElementDimension(source))
	end
)


addEvent('onClothesInit', true)
addEventHandler('onClothesInit', root,
	function()
		local result = {}
		local texture, model
		-- get all clothes
...
function setTimeFrozen(state)
	if state then
		...
		clientCall(root, 'setTimeFrozen', state, g_FrozenTime[1], g_FrozenTime[2], g_FrozenWeather)
	else
		if g_FrozenTime then
		...
		clientCall(root, 'setTimeFrozen', state)
	end
end
...
addEvent ( "onFreeroamEnter", true )
addEventHandler ( "onFreeroamEnter", getRootElement(), 
	function ( _, acc)
		local acc = getPlayerAccount( source )
		if acc then
			--GIVE MONEY
			local playerMoney = tonumber( getAccountData( acc, "freeroam.money" ) ) or 1000
	...
	function ( acc, _)
		if getElementData ( source, "gamemode" ) ~= "freeroam" then return end
			...
			if ( acc ) then
				...
				local pos = x..' '..y..' '..z
				setAccountData ( acc, "freeroam.money", playerMoney )
			end
		end
)
