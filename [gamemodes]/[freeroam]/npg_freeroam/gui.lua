...
classInfo = {
	...
	lst = {className = 'GridList', width = 80, height = 40},
	img = {className = 'StaticImage'},
	cbb = {className = 'ComboBox', width = 155, height = 100}
}
...
function createWindow(wnd, rebuild)
	if wnd.element then
		if rebuild then
			...
			Animation.createAndPlay(wnd.element, Animation.presets.guiFadeIn(300))
...
function _buildWindow(wnd, baseWnd, parentWnd)
	...
	if wndClass == 'wnd' then
		...
		guiSetAlpha(elem, 0)
		setTimer(
			function() Animation.createAndPlay(elem, Animation.presets.guiFadeIn(300)) end,
		300, 1) 
	...
	elseif wndClass == 'cbb' then
		elem = guiCreateComboBox ( relX, relY, relWidth, relHeight, true, parentWnd.element)
		if wnd.comboxes then
			for i, combobox in ipairs(wnd.comboboxes) do
				guiComboBoxAddItem(elem, column.text or column.attr or '', column.width or 0.9)
			end
		end
...
function closeWindow(...)
	...
	for i=1,#args do
		...
		Animation.createAndPlay(args[i].element, Animation.presets.guiFadeOut(300))
		if args[i].onclose then
			args[i].onclose()
		end
		setTimer(guiSetVisible, 300, 1, args[i].element, false)
	end
...
function hideAllWindows()
	...
	for wnd,_ in pairs(g_openedWindows) do
		setTimer(guiSetVisible,300,1,wnd.element,false)
        Animation.createAndPlay(wnd.element, Animation.presets.guiFadeOut(300))
	end
end
