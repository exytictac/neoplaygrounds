...
function onClientColShapeHitx( hitPlayer ) 
	...
	if( shops[num].Marker == nil ) then
		...
		setElementDimension( shops[num].Marker, _local )
	end
end
...
function onClientResourceStartx( res )
	if getElementData ( _local, "gamemode" ) == "rpg" then
...
addEventHandler( "onClientRPGEnter", root, onClientResourceStartx )
addEventHandler( "onClientResourceStart", resourceRoot, onClientResourceStartx )
