	...
	if type( name ) == "string" then
		...
		if forElement == getRootElement() then
			if not (priority > MAX_PRIRORITY_SLOT or priority < 1) then
				...
				return triggerClientEvent( getRootElement(), "doRPGboardAddColumn", getRootElement(), name, width, friendlyName, priority, sourceResource )
			end
		else
			return triggerClientEvent( forElement, "doRPGboardAddColumn", getRootElement(), name, width, friendlyName, priority, sourceResource )
		end
	end
	return false
end
		...
		if forElement == getRootElement() then
			for key, value in ipairs( scoreboardColumns ) do
				if name == value.name then
					...
					return triggerClientEvent( getRootElement(), "doRPGboardRemoveColumn", getRootElement(), name )
				end
			end
		else
			return triggerClientEvent( forElement, "doRPGboardRemoveColumn", getRootElement(), name )
		end
	end
	return false
end
...
function scoreboardClearColumns( forElement )
	...
	if forElement == getRootElement() then
		...
		return triggerClientEvent( getRootElement(), "doRPGboardClearColumns", getRootElement() )
	else
		return triggerClientEvent( forElement, "doRPGboardClearColumns", getRootElement() )
	end
end
...
function scoreboardResetColumns( forElement )
	...
	if forElement == getRootElement() then
		...
		local result = triggerClientEvent( getRootElement(), "doRPGboardResetColumns", getRootElement() )
		if result then
		...
		return triggerClientEvent( forElement, "doRPGboardResetColumns", getRootElement(), false )
	end
end
...
function scoreboardSetForced( forced, forElement )
	if type( forced ) == "boolean" then
		forElement = iif( type( forElement ) == "userdata" and isElement( forElement ), forElement, getRootElement() )
		return triggerClientEvent( forElement, "doRPGboardSetForced", getRootElement(), forced )
	else
		return false
	end
end
...
function scoreboardSetSortBy( name, desc, forElement )
	if type( name ) == "string" or name == nil then
		...
		return triggerClientEvent( forElement, "doRPGboardSetSortBy", getRootElement(), name, desc )
	else
		return false
	end
end
		...
		if not (priority > MAX_PRIRORITY_SLOT or priority < 1) then
			...
			if forElement == getRootElement() then
				...
				if columnIndex then
					...
					table.sort( scoreboardColumns, function ( a, b ) return a.priority < b.priority end )
					return triggerClientEvent( forElement, "doRPGboardSetColumnPriority", getRootElement(), name, priority )
				end
			else
				return triggerClientEvent( forElement, "doRPGboardSetColumnPriority", getRootElement(), name, priority )
			end
...
function onClientDXScoreboardResourceStart()
	for key, column in ipairs( scoreboardColumns ) do
		triggerClientEvent( client, "doRPGboardAddColumn", getRootElement(), column.name, column.width, column.friendlyName, column.priority )
	end
end
